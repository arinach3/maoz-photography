...
export default function Events(){
  ...
  const uploadImages = async () => {
    ...
    try {
      for (const file of files) {
        const storagePath = `galleries/events/images/${Date.now()}-${file.name}`;
        const storageRef = ref(storage, storagePath);

        await uploadBytes(storageRef, file);

        await addDoc(
          collection(db, "galleries", "events", "images"),
          {
            filename: file.name,
            storagePath,
  ...
  try {
    ...
    await deleteDoc(
      doc(db, "galleries", "events", "images", img.id)
    );
